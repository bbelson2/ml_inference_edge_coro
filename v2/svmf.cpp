...
#ifndef FLOATING_POINT_BITS
#define FLOATING_POINT_BITS 32
#endif

#if (FLOATING_POINT_BITS == 32)
typedef float float_unit_t;
#elif (FLOATING_POINT_BITS == 64)
typedef double float_unit_t;
#else
#error FLOATING_POINT_BITS values other than 32 and 64 not supported
#endif
...
public:
  ...
  inline static const char* description = "Test bed for implementation of SVM using fixed-point numbers, investigating the impact of cache prefetch using coroutines";
  inline static const char* version = "1.2";
};

#define PEM_SETUP_WHICH 0

template<typename PARAMS_T>
class output_model_svm
{
public:
...
public:
  output_model_svm(const vec_t &weights, unit_t bias) : weights_(weights), bias_(bias) {}
  ...
  {
    unit_t sum = (unit_t)bias_;
    const unit_t *w = &weights_[0];
    for (size_t j = 0; j < cp.width; j++, item_ptr++, w++)
    {
      sum += *item_ptr * *w;
    }
    next_index++;
    *output_ptr = (sum > 0) ? 1 : -1;
  }
...
int main(int ac, char **av)
{
  ...
  context_t<svmf_params> context(params);
  context.init_output_stream();
  context.init_telemetry(PEM_SETUP_WHICH);
  context.report_line_header();

  // Simulated data
  svmf_params::image_vec_t in_batch;
  svmf_params::tensor_t out_batch, control_out_batch;
  svmf_params::vec_t weights;    
  svmf_params::unit_t bias;
  create_data_floating_point(params, in_batch, out_batch, &weights, &bias);

  // Avoid 1st-time effects
  output_model_svm<svmf_params> om(weights, bias);
  context.before_test();
