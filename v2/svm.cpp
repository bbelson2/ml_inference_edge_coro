...
int main(int ac, char **av)
{
  ...
  context_t<svm_params> context(params);
  context.init_output_stream();
  context.init_telemetry(PEM_SETUP_WHICH);
  context.report_line_header();

  // Simulated data
  svm_params::image_vec_t in_batch;
  svm_params::tensor_t out_batch, control_out_batch;
  svm_params::vec_t weights;    
  svm_params::unit_t bias;
  create_data_fixed_point(params, in_batch, out_batch, &weights, &bias);

  // Avoid 1st-time effects
  output_model_svm<svm_params> om(weights, bias);
  context.before_test();
