...
public:
  ...
  inline static const char* description = "Test bed for implementation of Btree+ leaf visit algorithm, investigating the impact of cache prefetch using coroutines";
  inline static const char* version = "1.1";
};

#define PEM_SETUP_WHICH 0

template<typename PARAMS_T>
class output_model_leaf
{
public:
...
int main(int ac, char **av)
{
  ...
  context_t<leaf_params> context(params);
  context.init_output_stream();
  context.init_telemetry(PEM_SETUP_WHICH);
  context.report_line_header();

  // Simulated data
  leaf_params::image_vec_t in_batch;
  leaf_params::tensor_t out_batch, control_out_batch;
  create_data_int_unclustered(params, in_batch, out_batch, true);

  // Avoid 1st-time effects
  output_model_leaf<leaf_params> om;
  context.before_test();
