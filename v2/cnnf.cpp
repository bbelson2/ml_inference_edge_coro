...
int main(int ac, char **av)
{
  ...
  context_t<cnnf_params> context(params);
  context.init_output_stream();
  context.init_telemetry(PEM_SETUP_WHICH);
  context.report_line_header();

  // Simulated data
  cnnf_params::image_vec_t in_batch;
  cnnf_params::tensor_t out_batch, control_out_batch;
  cnnf_params::image_t kernel;
  create_data_floating_point(params, in_batch, out_batch, 
    (cnnf_params::image_t*)nullptr, (cnnf_params::unit_t*)nullptr, 
    &kernel, 3);
  //std::cout << "Kernel: " << join(kernel, ',') << "\r\n";

  // Avoid 1st-time effects
  output_model_cnn<cnnf_params> om(kernel, params.width);
  context.before_test();
