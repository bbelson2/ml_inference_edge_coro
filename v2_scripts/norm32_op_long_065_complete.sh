...
# Completes incomplete run (under v2 folders)

# Note - this runs norm over all models 
...
do
  fn=norm32_op_065_n${height}
  echo Writing ${fn}_*.csv
  hdr=-x
  for tasks in {8..8..2}
  do
    for width in {716..1024..4}
    do
      echo ${height}/${tasks}/${width} of 40 80 100 200 400 800/2,4,6,8/4-1024
      ../output/gcc/norm32.out -e 10 -b 100 -c $tasks -s $width -n $height ${short} -p ${dims} -u cpu_cycles,instructions,d_cache_reads,d_cache_misses -o - -w $hdr -g -f >> ../results/../results/v2/${fn}_norm32_append.csv
