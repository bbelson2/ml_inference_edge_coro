...
do
  ...
  do
    ...
    do
      ...
      ../output/llvm/norm.out -r ${repeats} -e 10 -b 100 -c $tasks -s $width -n $height ${short} -p ${dims} -u cpu_cycles,instructions,d_cache_reads,d_cache_misses -o - -w $hdr -g -f >> ../results/v2/${fn}.csv
      hdr=
    done
  done
done
